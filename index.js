...
class BudgetTracker {
  ...
  render() {
    ...
    this.totalAmountContainer.innerHTML = '<span id="totalAmountLabel">현재 합계 금액: </span><span id="totalAmount">0</span>';
  ...
  updateCharts() {
    ...
    const percentages = categoryData.map(data =>
      ((data.amount / income) * 100).toFixed(2)
    );
  ...
  updateTotalAmount() {
    ...
    const totalAmount = this.categories.reduce((sum, category) => {
      return safetyAdd(sum, category.subcategories.reduce((subSum, sub) => {
...
class Category {
  ...
  updatePercentage(income) {
    const total = this.subcategories.reduce((sum, sub) => safetyAdd(sum, (parseFloat(sub.amountInput.value) * 10000 || 0)), 0);
